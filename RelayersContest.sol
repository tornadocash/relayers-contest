...
contract RelayersContest {
    ...
    function register(string memory _ipOrDomain) public {
        require(now <= endRegistration, 'registration is over');
        require(!participants[_ipOrDomain], 'already registred');
        participants[_ipOrDomain] = true;
        participantsArray.push(_ipOrDomain);
        emit Register(_ipOrDomain);
    }
